...
CacheerPHP 6 is built from small, composable pieces. You interact with a `Cacheer`
...
- `Cacheer::get($key, $default)` returns the value or your default. Simple.
- `Cacheer::entry($key)` returns a [`CacheEntry`](./option-builder.md#cacheentry)
  so you can distinguish a stored `null` from a miss and read timestamps and
  remaining TTL.

## Where configuration lives

There is no ambient configuration. A `Cacheer` is exactly what you construct:
